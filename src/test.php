...
$user->setUsername('DS:ZFDO003');
$user->setPassword('DELTA045');
$user->setRealm('');
...
try {
    if ($useXML) {
        $parameters = new Dataworks\Enterprise\RequestRecordAsXml($user, $request, 0);
        $response = $service->RequestRecordAsXml($parameters);
        if ($echoData) {
            echo $response->getRequestRecordAsXmlResult()->getAny();
        }
    } else {
        ...
        if ($record->getStatusCode() == DataworksEnterprise\StatusType::Connected) {
         ...
         * if ($echoData) {
         * var_dump($record);
         * //echo $response->getRequestRecordResult()->getAny();
         * }
         */
    }
